...
> **Стек:** Claude Code как харнесс | 4 собственных MCP-сервера (22 инструмента) | политики как код | BM25-поиск и интеграция с Telegram без внешних зависимостей
...
mcp_servers/       4 MCP-сервера = руки агента
  ...
  telegram/        эскалация и приём ответа инженера          5 инструментов
...
|---|---|---|
| Решение без `kb_article_ids` отклоняется | `reply_to_ticket` | нет источника - нет инструкции |
| Уведомление о статусе - только после реального действия | `reply_to_ticket` | иначе это лазейка для ответа без источника |
| Заявку закрывает только ответ с решением | `guardrails.yaml` | уточняющий вопрос не итог работы |
...
Очередь заявок приходит из внешнего API. Корпоративные системы вокруг неё (Wiki и GitHub) смоделированы локально: так прогоны воспроизводимы и не зависят от доступности внешних сервисов. Telegram работает и как макет, и как настоящий канал. Граница проведена явно, чтобы было понятно, какие выводы из метрик правомерны.

| Сущность | Происхождение |
|---|---|
| Очередь заявок, категории, приоритеты, статусы | **реальные** - внешний API, 100 заявок, 23 уникальных описания |
| Канал Telegram | **реальный при заданном токене**, иначе макет - см. раздел ниже |
...
| Ответы инженеров L2 | смоделировано, если Telegram не подключён - тестовое окружение на 2 заявки |
...
| Двусторонний канал Telegram | готово, включается токеном |
| Реальная запись в очередь заявок (`LIVE_WRITES`) | адаптер написан, по умолчанию выключен |
...
```bash
make demo-seed   # 4 заявки под все сценарии: 901-904
make demo-clean  # чистый старт: сброс состояния + свежие демо-заявки
```

Заголовки, категории и приоритеты у них намеренно проставлены неверно - видно, как агент опирается на текст обращения. Заявки 902 и 904 содержат одно и то же обращение: первая уходит на вторую линию, вторая закрывается сама после того, как агент оформит ответ инженера статьёй. `demo-clean` удаляет только черновики, которых нет в git - содержимое репозитория не трогается.

---

## Подключение реального Telegram

По умолчанию эскалации складываются в `data/telegram/messages.json`, а ответы инженеров берутся из тестового окружения. Так прогон воспроизводится у любого без токенов и без сети - это важно для метрик.

Реальный канал включается двумя переменными окружения. Тогда эскалация уходит настоящим сообщением, а ответ инженера возвращается **реплаем** и подхватывается агентом.

**1. Завести бота.** В Telegram открыть `@BotFather` → `/newbot` → получить токен.

**2. Написать боту любое сообщение.** Без этого он не имеет права отвечать - ограничение Bot API, а не проекта.
...
`.env` в `.gitignore`, токен читается только локально. Проверить связь до запуска агента:
...
| | Макет (по умолчанию) | Реальный Telegram |
|---|---|---|
| Отправка | запись в `messages.json` | сообщение через Bot API |
| Ответ инженера | фикстура на 2 заявки | реплай на сообщение бота |
| Связь ответа с заявкой | по номеру в фикстуре | по `reply_to_message_id`, запасной вариант - тег `#номер` в тексте |
| Нужны токены и сеть | нет | да |

Три решения, которые стоит знать:

- **Отсутствие `.env` ничего не ломает.** Тот же приём, что у `LIVE_WRITES` в записи тикетов: демо обязано воспроизводиться без внешних сервисов, реальная интеграция включается флагом.
- **Сбой доставки не роняет эскалацию.** Ошибка Telegram уходит в журнал отдельным событием, заявка обрабатывается дальше, в ответе инструмента `delivered: false`. Внешний сервис не должен уметь останавливать процесс поддержки.
- **Входящие копятся в `data/telegram/inbox.json`.** `getUpdates` отдаёт каждое обновление ровно один раз, поэтому они складываются локально: повторный вызов инструмента не теряет ответ инженера. Файл в `.gitignore` - в нём имена и идентификаторы участников переписки.

Интеграция написана на `urllib` из стандартной библиотеки: внешних зависимостей у проекта по-прежнему две - `mcp` и `PyYAML`.
...
| [telegram](mcp_servers/telegram/server.py) | эскалация, ответственный подбирается по `routing.yaml`, а не выбирается моделью. Здесь же двусторонний канал: отправка и приём ответа инженера |
| [common/env.py](common/env.py) | чтение `.env` двенадцатью строками, без внешних зависимостей |
