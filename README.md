...
| Готово | В работе |
|---|---|
| 4 MCP-сервера, 22 инструмента | `.mcp.json` - регистрация серверов |
| 4 файла политик | `.claude/agents/l1-support.md` |
| Слой данных: overlay + аудит | `.claude/skills/` - процедуры L1 |
| BM25-поиск, 14 статей базы | `agent/run_queue.py` - headless-прогон |
| Reference set и метрики поиска | `eval/eval_agent.py` - метрики уровня 2 |
...
Агент внутри Claude Code - MCP-серверы подхватываются из `.mcp.json`, процедуры из `.claude/skills/`:
